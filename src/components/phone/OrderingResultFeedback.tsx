...
interface OrderingResultFeedbackProps {
  correctCount: number;
  totalItems: number;
  points: number;
  totalScore: number;
...
export default function OrderingResultFeedback({
  correctCount,
  totalItems,
  points,
  totalScore,
}: OrderingResultFeedbackProps) {
  const perfect = correctCount === totalItems;
  ...
  let heading = `${correctCount} of ${totalItems} Correct`;
